...
import { conversationService, messageService, personaService } from "../services/index.js";
...
function formatStyleGuidelines(guidelines?: Record<string, unknown> | null): string {
  if (!guidelines || Object.keys(guidelines).length === 0) {
    return '';
  }

  const formattedEntries = Object.entries(guidelines)
    .map(([key, value]) => {
      const normalizedKey = key.replace(/_/g, ' ');

      if (Array.isArray(value)) {
        return `- ${normalizedKey}: ${value.join(', ')}`;
      }
  ...
  return `\n\n**INSTRUÇÕES DE ESTILO DA PERSONA:**\n${formattedEntries}`;
}
...
function buildToolMetadata(
  ...
  const toolCalls = Array.isArray(response.toolCalls)
    ? response.toolCalls.map((call: any) => ({
        id: call?.toolCallId ?? call?.id ?? call?.toolName ?? call?.name,
        name: call?.toolName ?? call?.name ?? 'tool',
        input: call?.args ?? call?.input ?? null
      }))
    : undefined;

  const toolResults = Array.isArray(response.toolResults)
    ? response.toolResults.map((result: any) => ({
        name: result?.toolName ?? result?.id ?? 'tool',
...
async function generateContextualPrompt(whatsappNumber: string, currentMessage: string): Promise<{
  ...
  try {
    const { persona, profile: ensuredProfile } = await personaService.ensurePersonaForUser(whatsappNumber, {
      assignedBy: 'contextual-prompt',
      reason: 'ensure-persona-before-response'
    });

    const userProfile = ensuredProfile ?? await onboardingService.getUserProfile(whatsappNumber);
    const personaTools = personaService.getToolsForPersona(persona);
    ...
    // 3. MONTAR PROMPT DO SISTEMA COM TODAS AS INFORMAÇÕES
    let enhancedSystemPrompt = persona?.system_prompt ?? DALIO_AI_PROMPT;

    if (persona?.style_guidelines) {
      enhancedSystemPrompt += formatStyleGuidelines(persona.style_guidelines);
    }
...
- Estilo de Comunicação Preferido: ${userProfile.communication_style ?? 'Amigão que puxa papo'}
- Formato de Conteúdo Favorito: ${userProfile.content_format_preference ?? 'Mensagens curtas'}
- Frequência Desejada de Contato: ${userProfile.engagement_frequency ?? 'Sob demanda'}
- Estilo de Aprendizado: ${userProfile.learning_style ?? 'Aprende melhor com exemplos práticos'}
- Temas de Interesse: ${(userProfile.interest_tags ?? []).join(', ') || 'vamos descobrir juntos'}
...
Use essas informações para personalizar suas respostas e dar dicas mais relevantes para o perfil do usuário.`;
    ...
    if (persona?.default_disclaimer) {
      enhancedSystemPrompt += `\n\n**LEMBRE-SE:** ${persona.default_disclaimer}`;
    }
...
async function saveUserMessage(whatsappNumber: string, messageText: string, messageType: string = 'text'): Promise<void> {
  try {
    await personaService.ensurePersonaForUser(whatsappNumber, {
      assignedBy: 'message-log',
      reason: 'user-message'
    });
...
async function generateAudioWithElevenLabs(
  ...
  try {
    ...
    const audioDir = process.env.NODE_ENV === 'production' ? '/tmp/audio' : './audio';
  ...
  try {
    ...
    if (messages && messages.length > 0) {
      for (const message of messages) {
        ...
        const { systemPrompt, userPrompt, persona, tools: personaTools } = await generateContextualPrompt(from, messageText);

        const activeTools = personaTools && Object.keys(personaTools).length > 0 ? personaTools : undefined;

        const personaVoiceId = persona?.voice_id ?? defaultVoiceId;
        const personaMetadata = persona
          ? { personaId: persona.id, personaSlug: persona.slug }
          : undefined;
        ...
        if (
          ...
          try {
            ...
            const messageMetadata = buildToolMetadata(response, personaMetadata);

            // Gerar e enviar áudio com ElevenLabs
            await generateAndSendAudio(from, response.text, personaVoiceId);
            ...
            const response = await generateText({
              ...
              await saveBotMessage(
                ...
                undefined,
                response.usage?.totalTokens,
                buildToolMetadata(response, personaMetadata)
              );
            } catch (fallbackError) {
          ...
          const response = await generateText({
            ...
            await saveBotMessage(from, errorMessage, "error", false, undefined, undefined, personaMetadata);
          }
        }
      }
    }
